...
export class ProductService {
  ...
  protected convertDateFromClient(product: IProduct): IProduct {
    const copy: IProduct = Object.assign({}, product, {
      dateAdded: product.dateAdded && product.dateAdded.isValid() ? product.dateAdded.format(DATE_FORMAT) : undefined,
      dateModified: product.dateModified && product.dateModified.isValid() ? product.dateModified.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((product: IProduct) => {
        product.dateAdded = product.dateAdded ? moment(product.dateAdded) : undefined;
        product.dateModified = product.dateModified ? moment(product.dateModified) : undefined;
      });
    }
    return res;
  }
